...
<script>
  export default {
    ...
    methods: {
      ...
      const box = container.querySelector(".floating-box");

      const getRandomPosition = () => Math.floor(Math.random() * 80) + "vh";
      const moveBox = () => {
  ...
  // 每隔一段时间执行移动
  setInterval(moveBox, 1); // 调整移动的时间间隔，单位为毫秒
  ...
  const recalculatePosition = () => {
    ...
    getRandomPosition = () => Math.floor(Math.random() * containerWidth) + "px";
    moveBox(); // 移动到新的随机位置
  };
